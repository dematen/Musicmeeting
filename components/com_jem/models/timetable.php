 ...
 * @version 1.9.5
 * @package JEM
 * @copyright (C) 2014 Steven Trooster / Ghost art digital media
...
/**
 * JEM Component Timetable Model
 *
 * @package JEM
 *
 */
class JEMModelTimetable extends JModelLegacy
{
	/**
	 * Events data array
	 *
	...
	function __construct()
	{
		...
		$limit      = $app->getUserStateFromRequest('com_jem.timetable.limit', 'limit', $jemsettings->display_num, 'int');
		$limitstart = $app->getUserStateFromRequest('com_jem.timetable.limitstart', 'limitstart', 0, 'int');
	...
	function _buildQuery()
	{
		...
		$query = 'SELECT DISTINCT a.id, a.dates, a.enddates, a.times, a.endtimes, a.title, a.created, a.locid, a.fulltext, TIME_FORMAT(times, "%H") AS h, TIME_FORMAT(times, "%i") AS m, '
	...
	function _buildOrderBy()
	{
		...
		$filter_order		= $app->getUserStateFromRequest('com_jem.timetable.filter_order', 'filter_order', 'a.dates', 'cmd');
		$filter_order_Dir	= $app->getUserStateFromRequest('com_jem.timetable.filter_order_Dir', 'filter_order_Dir', 'ASC', 'word');
	...
	function _buildWhere()
	{
		...
		$filter 		= $app->getUserStateFromRequest('com_jem.timetable.filter', 'filter', '', 'int');
		$search 		= $app->getUserStateFromRequest('com_jem.timetable.filter_search', 'filter_search', '', 'string');
